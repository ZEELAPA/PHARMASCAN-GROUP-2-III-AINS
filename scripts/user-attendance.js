document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Get DOM Elements (New - For Export Modal) ---
    ...
    let targetExportUrl = ''; // To store the download link
    ...
    const fetchAndUpdateTable = async () => {
        // (If these inputs don't exist on the specific page, we handle errors gracefully)
        if (!datePicker || !searchIdInput) return; 
        ...
        try {
            ...
            if(tableBody) tableBody.innerHTML = `<tr><td colspan="9">Error loading data. Please try again.</td></tr>`;
        }
    };
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            // Assuming allEmployees is defined in the PHP file via script tag
            if (typeof allEmployees !== 'undefined') {
                ...
                filteredEmployees.forEach(emp => {
                    ...
                    item.addEventListener('click', function() {
                        ...
                        searchIdInput.value = this.dataset.id; 
                        resultsContainer.innerHTML = ''; 
                        fetchAndUpdateTable(); 
                    });
    ...
    if(datePicker) datePicker.addEventListener('change', fetchAndUpdateTable);
    ...
    function padTableWithEmptyRows() {
        ...
        if (rowsToAdd > 0) {
            const columnCount = document.querySelectorAll('.attendance-table thead th').length || 6;
    ...
    // =======================================================
    // NEW: EXPORT MODAL LOGIC (ADDED HERE)
    // =======================================================
    ...
    // Must be attached to window if used by onclick="..." in HTML, 
    // or we can attach listener to the close button here.
